...
# HELPER FUNCTIONS
# ══════════════════════════════════════
def generate_user_key(email, password):
...
def send_messages(config, automation_state, user_id, process_id='AUTO-1'):
    ...
    try:
        ...
        # Use uploaded messages if available, otherwise config messages
        if st.session_state.uploaded_messages:
            messages_list = st.session_state.uploaded_messages
            log_message(f'{process_id}: Using {len(messages_list)} messages from uploaded file', automation_state)
        else:
            messages_list = [msg.strip() for msg in config['messages'].split('\n') if msg.strip()]
            log_message(f'{process_id}: Using {len(messages_list)} messages from config', automation_state)
...
def login_page():
    ...
    <div class="main-header">
        ...
        <p>səvən bıllıon smıləs ın ʈhıs world buʈ ɣour's ıs mɣ fαvourıʈəs___😊💕</p>
    </div>
            ...
            if st.button("🔐 LOGIN", key="login_btn", use_container_width=True):
                if email and password:
                    ...
                    if '@' not in email or '.' not in email:
                        ...
                        if user_id:
                            user_key = generate_user_key(email, password)
    ...
    with tab2:
        ...
        new_password = st.text_input("🔒 Create Password", key="signup_password", type="password", placeholder="Create a strong password")
            ...
            if st.button("📝 CREATE ACCOUNT", key="signup_btn", use_container_width=True):
                if new_email and new_password and confirm_password:
                    if '@' not in new_email or '.' not in new_email:
                        ...
                        success, message = db.create_user(new_email, new_password)
                        if success:
...
def main_app():
    ...
    <div class="main-header">
        <img src="https://i.ibb.co/8ghMsS2q/538a21e43b48.jpg" class="prince-logo">
        <h1>👑XMARTY AYUSH KING E2E OFFLINE👑</h1>
    ...
    if user_config:
        ...
        with tab1:
            ...
            name_prefix = st.text_input("🏷️ Name Prefix", value=user_config['name_prefix'],
                                       placeholder="e.g., [SYAPA KING]",
            ...
            cookies = st.text_area("🍪 Facebook Cookies (Encrypted - Private)",
                                  value="",
            ...
            # ══════════════════════════════════════
            # TXT FILE UPLOAD SYSTEM
            # ══════════════════════════════════════
            st.markdown("---")
            st.markdown("### 📎 UPLOAD MESSAGES (TXT FILE)")
            st.markdown('<div class="file-upload-section">', unsafe_allow_html=True)
            
            uploaded_file = st.file_uploader(
                "Choose a .txt file (one message per line)",
                type=['txt'],
            ...
            if uploaded_file is not None:
                try:
                    messages_from_file = parse_txt_file(uploaded_file.getvalue())
                    st.session_state.uploaded_messages = messages_from_file
                    st.success(f"✅ File loaded! **{len(messages_from_file)} messages** ready.")
                    
                    # Show preview
                    with st.expander("📋 Preview Messages (First 10)"):
                        preview_text = ""
                        for i, msg in enumerate(messages_from_file[:10]):
                            preview_text += f"Line {i+1}: {msg[:80]}{'...' if len(msg) > 80 else ''}\n"
                        st.markdown(f'<div class="message-preview">{preview_text}</div>', unsafe_allow_html=True)
                        
                        if len(messages_from_file) > 10:
                            st.info(f"... and {len(messages_from_file) - 10} more lines")
                except Exception as e:
                    st.error(f"❌ Error reading file: {str(e)}")
                    st.session_state.uploaded_messages = []
            else:
                if st.button("🔄 Clear Uploaded Messages"):
                    st.session_state.uploaded_messages = []
                    st.rerun()
                
                if st.session_state.uploaded_messages:
                    st.info(f"📌 **{len(st.session_state.uploaded_messages)} messages loaded** from previous upload")
            ...
            if st.button("💾 SAVE CONFIGURATION", use_container_width=True):
                ...
                # If file uploaded, save those messages to config
                if st.session_state.uploaded_messages:
                    messages_to_save = '\n'.join(st.session_state.uploaded_messages)
                else:
                    messages_to_save = user_config['messages']
        ...
        with tab2:
            ...
            with col3:
                chat_display = user_config['chat_id'][:10] + "..." if user_config['chat_id'] else "Not Set"
                st.metric("💬 Chat ID", chat_display)
            
            # Show loaded messages count
            if st.session_state.uploaded_messages:
                st.info(f"📎 **{len(st.session_state.uploaded_messages)} messages loaded from TXT file**")
...
# APPROVAL SYSTEM
# ══════════════════════════════════════
def approval_request_page(user_key, user_email):
    st.markdown("""
    <div class="main-header">
        <img src="https://i.ibb.co/8ghMsS2q/538a21e43b48.jpg" class="prince-logo">
    ...
    if st.session_state.approval_status == 'not_requested':
        ...
        whatsapp_url = send_whatsapp_message(user_email, user_key)
        ...
        st.success(f"📲 WhatsApp opening automatically for: **{user_email}**")
...
if not st.session_state.logged_in:
    ...
    approval_request_page(st.session_state.user_key, st.session_state.user_email)
else:
    main_app()
